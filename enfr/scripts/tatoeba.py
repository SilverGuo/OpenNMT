...
# load data
dfBase = pd.read_csv(os.path.join(args.data_dir, 'sentences.csv'), sep='\t', header=None, names=['label', 'lang', 'text'])
dfBase = dfBase.set_index('label')
dfLink = pd.read_csv(os.path.join(args.data_dir, 'links.csv'), sep='\t', header=None, names=['ida', 'idb'])
...
# save data
f_en = open(os.path.join(args.data_dir, 'tatoeba.en'), 'w')
f_fr = open(os.path.join(args.data_dir, 'tatoeba.fr'), 'w')
